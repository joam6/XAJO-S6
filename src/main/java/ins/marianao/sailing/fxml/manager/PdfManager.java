...
public class PdfManager implements IEventHandler {
	...
	public static final String HEADER_IMAGE = "logo-gene.jpg";
	...
	public PdfManager(String outputPdfFile) throws Exception {
		...
		try {
			this.headerFont = PdfFontFactory.createFont(StandardFonts.HELVETICA);
	...
	public void generarPDF( String titol, String htmlContent, boolean landscape )  throws Exception {
		...
		try {
			...
			htmlPre += "		* {";
			htmlPre += "		    font-family: '"+StandardFonts.HELVETICA+"';";  // 'FreeSerif' o 'FreeMono'
			...
			//converterProperties.setBaseUri(ResourceManager.BASE_URI);
			Document document = HtmlConverter.convertToDocument(htmlPre+htmlContent+htmlPost, this.pdfDocument, converterProperties);
			//Document document = HtmlConverter.convertToDocument(htmlPre+"Hola"+htmlPost, this.pdfDocument, converterProperties);
			//Document document = HtmlConverter.convertToDocument(htmlPre+htmlContent+htmlPost, this.pdfDocument, converterProperties);
			//HtmlConverter.convertToPdf(htmlPre+htmlContent+htmlPost, this.pdfDocument, converterProperties);
			...
			e.printStackTrace();
			throw new Exception(ResourceManager.getInstance().getText("error.pdf")+" "+e.getMessage());
		}
	}
	...
	public void handleEvent(Event event) {
		...
		Image logo = null;
		try {
			logo = new Image(ImageDataFactory.create(ResourceManager.BASE_URI+HEADER_IMAGE));
